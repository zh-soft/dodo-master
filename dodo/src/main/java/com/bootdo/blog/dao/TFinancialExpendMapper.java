...
import com.bootdo.blog.domain.TFinancialExpendDO;
 ...
 */
@Mapper
public interface TFinancialExpendMapper {

	@Select("select `id`, `user_id`, `expend_type`, `expend_amount`, `unit`, `expend_status`, `expend_date`, `description`, `data_url`, `create_time`, `update_time`, `create_user`, `update_user` from t_financial_expend where id = #{id}")
	TFinancialExpendDO get(Long id);
	
	@Select("<script>" +
	"select * from t_financial_expend " + 
			"<where>" + 
			...
			"</if>"+
			"</script>")
	List<TFinancialExpendDO> list(Map<String,Object> map);
	
	@Select("<script>" +
	"select count(*) from t_financial_expend " + 
			"<where>" + 
		  		  "<if test=\"id != null and id != ''\">"+ "and id = #{id} " + "</if>" + 
		  		  "<if test=\"userId != null and userId != ''\">"+ "and user_id = #{userId} " + "</if>" + 
		  		  "<if test=\"expendType != null and expendType != ''\">"+ "and expend_type = #{expendType} " + "</if>" + 
	...
	@Insert("insert into t_financial_expend (`user_id`, `expend_type`, `expend_amount`, `unit`, `expend_status`, `expend_date`, `description`, `data_url`, `create_time`, `update_time`, `create_user`, `update_user`)"
	...
	int save(TFinancialExpendDO tFinancialExpend);
	...
	@Update("<script>"+ 
			"update t_financial_expend " + 
					...
					"where id = #{id}"+
			"</script>")
	int update(TFinancialExpendDO tFinancialExpend);
	
	@Delete("delete from t_financial_expend where id =#{id}")
	int remove(Long id);
	
	@Delete("<script>"+ 
			"delete from t_financial_expend where id in " + 
